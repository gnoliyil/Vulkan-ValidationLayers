...
class RenderPassAccessContext {
  public:
    ...
    bool ValidateDrawSubpassAttachment(const SyncValidator &sync_state, const CMD_BUFFER_STATE &cmd, const VkRect2D &render_area,
                                       const char *func_name) const;
    void RecordDrawSubpassAttachment(const CMD_BUFFER_STATE &cmd, const VkRect2D &render_area,
                                     const ResourceUsageTag &tag);
...
class SyncValidator : public ValidationStateTracker, public SyncStageAccess {
  public:
    ...
    void RecrodIndirectBuffer(AccessContext &context, const ResourceUsageTag &tag, const VkDeviceSize struct_size,
